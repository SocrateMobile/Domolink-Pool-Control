...
with open("examples/domolink_pool_official_app_analyse.yaml", "r") as f:
    analyse = f.read()

with open("examples/domolink_pool_official_app_controle.yaml", "r") as f:
    controle = f.read()
...
with open("examples/domolink_pool_official_app_combined.yaml", "w") as f:
    f.write(combined)

md_content = f"""# Cartes Lovelace "DomoLink Pool Control Official App"
...
> [!NOTE]
> **Détection automatique de votre appareil DomoLink Pool Control**
> Le code JavaScript intégré dans ces cartes est conçu pour rechercher automatiquement le nom exact de votre appareil DomoLink Pool Control dans Home Assistant (par exemple `sensor.jardin_domolink_pool_piscine_ph`).
> Vous n'avez pas besoin de renommer vos entités ou de modifier le code si votre appareil est dans une pièce spécifique (comme "Jardin") ou s'il s'appelle différemment, tant que les entités d'origine générées par l'intégration contiennent le mot "domolink_pool" !
