"""Update platform for DomoLink Pool Control integration."""
...
GITHUB_REPO = "SocrateMobile/Domolink-Pool-Control"
...
def get_installed_version() -> str:
    ...
    try:
        ...
        _LOGGER.debug("Could not read manifest.json version for domolink_pool: %s", err)
    return "5.7.3"
...
class DomolinkPoolUpdateEntity(UpdateEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"domolink_pool_update_{entry.entry_id}"
        self._attr_title = "DomoLink Pool Control"
        ...
        self._attr_device_info = DeviceInfo(
            ...
            model="DomoLink Pool Control Smart Pool Monitor",
    ...
    async def async_update(self) -> None:
        ...
        try:
            ...
            headers = {
                "User-Agent": "HomeAssistant-DomoLink Pool ControlPool",
            ...
            async with session.get(
                ...
                if getattr(self, "hass", None) and getattr(self, "entity_id", None):
                    try:
                        ...
                        pass
                _LOGGER.info(
                    "DomoLink Pool Control update check: installed=%s, latest=%s, update_available=%s",
                    self._attr_installed_version,
                    clean_tag,
                    has_update,
                )
        except asyncio.TimeoutError:
            _LOGGER.debug("Timeout checking GitHub releases for DomoLink Pool Control")
        except Exception as err:
            _LOGGER.warning("Error checking GitHub releases for DomoLink Pool Control: %s", err)
    ...
    async def async_install(
        ...
        _LOGGER.info(
            "Starting DomoLink Pool Control update to version %s (download: %s)",
        ...
        if backup and self.hass.services.has_service("domolink_backup", "create_backup"):
            _LOGGER.info("DomoLink Pool Control Pool: Déclenchement d'une sauvegarde de sécurité avant la mise à jour (via DomoLink-BackUp).")
            import datetime
            try:
                ...
                _LOGGER.warning("DomoLink Pool Control Pool: Impossible de lancer la sauvegarde pré-update: %s", e)

        temp_dir = tempfile.mkdtemp(prefix="domolink_pool_update_")
        zip_path = os.path.join(temp_dir, "release.zip")

        try:
            session = async_get_clientsession(self.hass)
            headers = {"User-Agent": "HomeAssistant-DomoLink Pool ControlPool"}
            async with session.get(self._zip_download_url, headers=headers, timeout=aiohttp.ClientTimeout(total=90)) as resp:
                if resp.status != 200:
                    raise RuntimeError(f"Échec téléchargement release DomoLink Pool Control (HTTP {resp.status})")
            ...
            def _do_extract_and_copy() -> None:
                ...
                for root, _dirs, _files in os.walk(extract_path):
                    if os.path.exists(os.path.join(root, "__init__.py")) and os.path.exists(
                        ...
                        try:
                            ...
                            if os.path.basename(root) in (DOMAIN, "domolink_pool", "DomoLink Pool Control-Pool"):
                                source_component_dir = root
                                break

                if not source_component_dir:
                    raise RuntimeError("L'archive ne contient pas de composant domolink_pool valide.")
                ...
                if backup:
                    ...
                    _LOGGER.info("DomoLink Pool Control safety backup saved to %s", backup_dir)
                ...
                _LOGGER.info("DomoLink Pool Control files successfully updated in %s", target_dir)
            ...
            if self.hass.services.has_service("restart_ha", "start_process"):
                _LOGGER.info("DomoLink Pool Control Pool: Utilisation de Restart-HA pour le redémarrage (Safe Reboot).")
                await self.hass.services.async_call("restart_ha", "start_process", {"action": "quick_restart"})
            else:
                _LOGGER.info("DomoLink Pool Control Pool: Redémarrage standard Home Assistant.")
